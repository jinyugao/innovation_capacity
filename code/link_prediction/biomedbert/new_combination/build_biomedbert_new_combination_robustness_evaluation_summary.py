"""Build BiomedBERT new-combination robustness evaluation summaries."""
...
RESULT_DIR = PROJECT_DIR / "results/link_prediction/biomedbert/new_combination/robustness"

ANNOTATED_PREDICATION_ROOT = (
    INTERIM_DIR
...
OUTPUT_FILE = RESULT_DIR / "biomedbert_new_combination_robustness_evaluation_summary.csv"
...
ANNOTATION_COLUMN = "biomedbert_new_combination_robustness_annotation"
...
def annotated_file_for_year(focal_year: int, percentile: int) -> Path:
    ...
    return (
        ...
        / label
        / (
            f"{ANNOTATED_FILE_PREFIX}_biomedbert_new_combination_robustness_top_"
...
def build_summary_row(focal_year: int, percentile: int) -> dict[str, object]:
    ...
    print(
        "Summarizing BiomedBERT new-combination robustness evaluation for "
...
def main() -> None:
    ...
    for focal_year in range(BASE_YEAR, BASE_YEAR + N_YEARS):
        for percentile in ROBUSTNESS_PERCENTILES:
            rows.append(build_summary_row(focal_year, percentile))

    summary = pd.DataFrame(rows)
    summary.to_csv(OUTPUT_FILE, index=False)
    print(
        "Saved BiomedBERT new-combination robustness evaluation summary to "
        f"{OUTPUT_FILE}"
    )
