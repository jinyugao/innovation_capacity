"""Annotate first-layer predications for BiomedBERT robustness thresholds."""
...
PREDICTED_EDGE_FILE_PREFIX = (
    "biomedbert_new_combination_robustness_predicted_edges_top"
)
...
OUTPUT_ANNOTATION_COLUMN = "biomedbert_new_combination_robustness_annotation"
...
ADDED_OUTPUT_COLUMNS = [
    ...
    "biomedbert_new_combination_robustness_method",
    "biomedbert_new_combination_robustness_top_percentile",
    "biomedbert_new_combination_robustness_predicted",
    "biomedbert_new_combination_robustness_score",
    "biomedbert_new_combination_robustness_rank_within_method_year",
]
...
def predicted_edge_file_for_year(focal_year: int, percentile: int) -> Path:
    ...
    return (
        PREDICTED_EDGE_ROOT
        / label
        / f"{PREDICTED_EDGE_FILE_PREFIX}_{label}_{focal_year}.csv.gz"
    )
...
def output_file_for_year(focal_year: int, percentile: int) -> Path:
    ...
    return (
        ...
        / label
        / (
            f"{OUTPUT_FILE_PREFIX}_biomedbert_new_combination_robustness_top_"
...
def annotate_chunk(
    ...
    annotated["biomedbert_new_combination_robustness_method"] = "biomedbert"
    annotated["biomedbert_new_combination_robustness_top_percentile"] = percentile
    annotated["biomedbert_new_combination_robustness_predicted"] = is_predicted_values
    annotated["biomedbert_new_combination_robustness_score"] = prediction_scores
    annotated[
        "biomedbert_new_combination_robustness_rank_within_method_year"
    ] = prediction_ranks
    return annotated
...
def annotate_focal_year_percentile(focal_year: int, percentile: int) -> None:
    ...
    print(
        "Annotating BiomedBERT new-combination robustness predictions for "
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        annotated_chunk = annotate_chunk(chunk, predicted_edges, percentile)
    ...
    print(f"Saved BiomedBERT new-combination robustness annotation to {output_file}")
