"""Calculate Adamic-Adar scores for generic two-hop candidate edges."""
...
OUTPUT_DIR = Path(
    "/xdisk/sebratt/jinyugao/projects/innovation_capacity/data/interim/"
    "adamic_adar_link_prediction/candidate_edges"
)
...
def output_file_for_year(focal_year: int) -> Path:
    return OUTPUT_DIR / f"adamic_adar_scored_candidate_edges_{focal_year}.csv.gz"
...
def score_candidate_edges(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        chunk["adamic_adar_score"] = [
            adamic_adar_score(node_a, node_b, adj)
    ...
    if not wrote_header:
        pd.DataFrame(
            columns=["node_a", "node_b", "pyear", "adamic_adar_score"]
        ).to_csv(output_file, index=False, compression="gzip")

    print(f"Saved Adamic-Adar scores to {output_file}")
    print(f"Total candidate edges scored: {total_rows:,}")


def main() -> None:
    focal_year = get_focal_year()
    print(f"Starting Adamic-Adar scoring for focal year {focal_year}.")
    adj = build_prior_five_year_network(focal_year)
    score_candidate_edges(focal_year, adj)
    print(f"Finished Adamic-Adar scoring for focal year {focal_year}.")
