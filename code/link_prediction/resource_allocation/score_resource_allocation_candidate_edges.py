"""Calculate resource-allocation scores for generic two-hop candidate edges."""
...
OUTPUT_DIR = Path(
    "/xdisk/sebratt/jinyugao/projects/innovation_capacity/data/interim/"
    "resource_allocation_link_prediction/candidate_edges"
)
...
def output_file_for_year(focal_year: int) -> Path:
    return OUTPUT_DIR / f"resource_allocation_scored_candidate_edges_{focal_year}.csv.gz"
...
def resource_allocation_score(
    ...
    score = 0.0
    for common_neighbor in neighbors_a:
        if common_neighbor not in neighbors_b:
            continue

        degree = len(adj[common_neighbor])
        if degree > 0:
            score += 1.0 / degree
...
def score_candidate_edges(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        chunk["resource_allocation_score"] = [
            resource_allocation_score(node_a, node_b, adj)
    ...
    if not wrote_header:
        pd.DataFrame(
            columns=["node_a", "node_b", "pyear", "resource_allocation_score"]
        ).to_csv(output_file, index=False, compression="gzip")

    print(f"Saved resource-allocation scores to {output_file}")
    print(f"Total candidate edges scored: {total_rows:,}")


def main() -> None:
    focal_year = get_focal_year()
    print(f"Starting resource-allocation scoring for focal year {focal_year}.")
    adj = build_prior_five_year_network(focal_year)
    score_candidate_edges(focal_year, adj)
    print(f"Finished resource-allocation scoring for focal year {focal_year}.")
