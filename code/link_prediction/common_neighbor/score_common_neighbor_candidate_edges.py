"""Calculate common-neighbor scores for generic two-hop candidate edges."""
...
def output_file_for_year(focal_year: int) -> Path:
    return OUTPUT_DIR / f"common_neighbor_scored_candidate_edges_{focal_year}.csv.gz"
...
def common_neighbor_score(
    ...
    for common_neighbor in neighbors_a:
        if common_neighbor in neighbors_b and (
            len(adj[common_neighbor]) <= MAX_COMMON_NEIGHBOR_DEGREE
        ):
            score += 1
...
def score_candidate_edges(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        chunk["common_neighbor_score"] = [
            common_neighbor_score(node_a, node_b, adj)
        ...
        print(
            f"Chunk {chunk_number:,}: scored {total_rows:,} candidate edges so far."
        )

    if not wrote_header:
        pd.DataFrame(
            columns=["node_a", "node_b", "pyear", "common_neighbor_score"]
        ).to_csv(output_file, index=False, compression="gzip")

    print(f"Saved common-neighbor scores to {output_file}")
    print(f"Total candidate edges scored: {total_rows:,}")


def main() -> None:
    focal_year = get_focal_year()
    print(f"Starting common-neighbor scoring for focal year {focal_year}.")
    adj = build_prior_five_year_network(focal_year)
    score_candidate_edges(focal_year, adj)
    print(f"Finished common-neighbor scoring for focal year {focal_year}.")
