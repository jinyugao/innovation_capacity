"""Calculate preferential-attachment scores for generic two-hop candidate edges."""
...
def output_file_for_year(focal_year: int) -> Path:
    return OUTPUT_DIR / f"preferential_attachment_scored_candidate_edges_{focal_year}.csv.gz"
...
def preferential_attachment_score(
    ...
    return len(adj.get(node_a, set())) * len(adj.get(node_b, set()))
...
def score_candidate_edges(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        chunk["preferential_attachment_score"] = [
            preferential_attachment_score(node_a, node_b, adj)
    ...
    print(f"Saved preferential-attachment scores to {output_file}")
    print(f"Total candidate edges scored: {total_rows:,}")


def main() -> None:
    focal_year = get_focal_year()
    print(f"Starting preferential-attachment scoring for focal year {focal_year}.")
    adj = build_prior_five_year_network(focal_year)
    score_candidate_edges(focal_year, adj)
    print(f"Finished preferential-attachment scoring for focal year {focal_year}.")
